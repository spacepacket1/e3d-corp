You are producing the "External Context" section of a repo findings report.
Keep it short and focused on the current state of the art relevant to the repo.

Return markdown bullet points or short paragraphs only.

Also include a distinct "### Analogous Patterns" subsection: pick the 2-3
most transferable entries from the "Analogy domains to consider" line in the
local facts below for this specific repo, and for each one name the source
domain, the mechanic being borrowed, and one concrete sentence on how it
could apply here. The goal is to seed ideation with cross-domain metaphors,
not just incremental extensions of what the repo already does.
