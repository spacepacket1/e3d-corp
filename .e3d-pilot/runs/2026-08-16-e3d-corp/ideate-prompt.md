You are the ideate stage of an autonomous repo-improvement pipeline.
Propose 3-5 ranked candidate ideas (most valuable first) for what this repo
should work on next, based on the findings and dedup context below.

For EACH candidate you must explicitly check it against the git branches, the
PR list (when available), and past runs candidates/specs listed below, and
state plainly whether it duplicates something that already exists.

This project's primary goal is attracting and retaining users; revenue is
secondary. Draw on the findings' "Analogous Patterns" subsection where it
applies, and consider ideas spanning multiple categories (data usefulness,
workflow-in-UI, social, gamification, testing, marketing, selling) when good
ones exist for each -- but never force a weak or duplicate idea into a
category just for coverage.

Respond using exactly this format, one block per candidate:

### Candidate <N>: <short title>
Duplicate: yes|no
Dedup rationale: <cite the specific branch/PR/run you checked, or say why not>
Category: data|workflow|social|gamification|testing|marketing|selling|other
Analogy: <source domain> -- <how it transfers, or "none" if not applicable>
Attraction (1-5): <score, new-user pull>
Retention (1-5): <score, repeat-engagement value>
Effort: low|medium|high
Revenue (1-5|n/a): <score, secondary -- never outranks Attraction+Retention>
Description: <what to build and why>

Rank non-duplicate candidates primarily by Attraction + Retention (highest
first); Revenue may only break ties between otherwise-equal candidates and
must never let a lower Attraction + Retention candidate outrank a higher one.

After all candidates, end your response with exactly this block: choose the
highest-ranked candidate that is NOT a duplicate as "selected", using the
literal value "none" only if every candidate is a duplicate of existing work.

---IDEATE-STATUS---
selected: candidate-<N> | none
reason: <one line>
...
```text
---
head_sha: 69fa3ba66bc8bca26a21b50370dbab6cc700fc71
focus: default
---
