...
// One chain can accumulate multiple snapshots over time (each recordOutcome
// call appends a fresh one) - this is the deduped view, latest snapshot per
// correlationId, for anything that aggregates "per chain" (Phase 10's
// metrics) rather than "per recorded-outcome-event."
export function listLatestExperience(dataDir) {
  const byCorrelationId = new Map();
  for (const record of listExperience(dataDir)) {
    byCorrelationId.set(record.correlationId, record);
  }
  return [...byCorrelationId.values()];
}
