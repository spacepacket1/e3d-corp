...
import { loadWebCredentials, checkBasicAuth, loadLeadWebhookToken, checkBearerToken } from './auth.js';
  ...
  return async function requestListener(req, res) {
    try {
      ...
      if (req.method === 'GET' && segments[0] === 'metrics' && segments.length === 1) {
        ...
        return sendHtml(res, 200, renderPage('Metrics', renderMetrics(computeMetrics(dataDir, { since }))));
      }
