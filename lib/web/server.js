...
import { renderPage, renderOpportunitiesList, renderOpportunityDetail, renderProposalsList, renderProposalDetail } from './render.js';
...
async function handleProposalDecide(req, res, dataDir, id, decisionValue, credentials) {
  const form = await requireCsrf(req, res);
  if (!form) return;
  ...
  let result;
  try {
    result = await decideProposal(dataDir, id, decisionValue, reason, credentials.user, 'web');
  } catch (error) {
...
async function handleProposalConfirm(req, res, dataDir, id, credentials) {
  const form = await requireCsrf(req, res);
  if (!form) return;

  try {
    await confirmAndExecute(dataDir, id, credentials.user, 'web');
  } catch (error) {
    return handleProposalDetail(req, res, dataDir, id, { error: error.message });
  }
  ...
  return async function requestListener(req, res) {
    try {
      ...
      if (
        ...
        return await handleProposalDecide(req, res, dataDir, segments[1], segments[2] === 'approve' ? 'approved' : 'rejected', credentials);
      }
      if (req.method === 'POST' && segments[0] === 'proposals' && segments.length === 3 && segments[2] === 'confirm') {
        return await handleProposalConfirm(req, res, dataDir, segments[1], credentials);
      }
