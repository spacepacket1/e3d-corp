import { AUTHORITY_LEVELS } from '../authority/policy.js';
import { OUTCOME_TYPES } from '../outcomes/schema.js';

const OPPORTUNITY_DECISIONS = ['reviewed', 'pursuing', 'no-value'];

export function escapeHtml(value) {
  return String(value ?? '').replace(/[&<>"']/g, (char) => {
    switch (char) {
      case '&':
        return '&amp;';
      case '<':
        return '&lt;';
      case '>':
        return '&gt;';
      case '"':
        return '&quot;';
      default:
        return '&#39;';
    }
  });
}

const STYLE = `
  body { font-family: -apple-system, system-ui, sans-serif; max-width: 900px; margin: 2rem auto; padding: 0 1rem; color: #1a1a1a; }
  nav a { margin-right: 1rem; }
  table { border-collapse: collapse; width: 100%; margin: 1rem 0; }
  th, td { text-align: left; padding: 0.4rem 0.6rem; border-bottom: 1px solid #ddd; font-size: 0.9rem; }
  .chain li { font-family: ui-monospace, monospace; font-size: 0.85rem; }
  form.inline { display: inline-block; margin-right: 0.5rem; }
  fieldset { margin: 1rem 0; }
  .error { color: #b00020; }
  .badge { display: inline-block; padding: 0.1rem 0.5rem; border-radius: 0.3rem; background: #eee; font-size: 0.8rem; }
`;

export function renderPage(title, bodyHtml) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>${escapeHtml(title)} - e3d-corp</title>
<style>${STYLE}</style>
</head>
<body>
<nav><a href="/opportunities">Opportunities</a><a href="/proposals">Proposals</a><a href="/actions">Actions</a><a href="/outcomes">Outcomes</a><a href="/metrics">Metrics</a></nav>
<h1>${escapeHtml(title)}</h1>
${bodyHtml}
</body>
</html>`;
}

function csrfField(csrfToken) {
  return `<input type="hidden" name="_csrf" value="${escapeHtml(csrfToken)}">`;
}

function renderPursuitState(opportunity) {
  return opportunity?.pursuable === true ? 'pursuable' : 'intelligence-only';
}

function renderCounterpartySummary(counterparty) {
  if (!counterparty) {
    return 'unknown (predates this field)';
  }
  if (counterparty.kind === 'none') {
    return 'none';
  }
  return counterparty.name ?? 'unknown';
}

function renderCounterpartyDetail(counterparty) {
  if (!counterparty) {
    return 'unknown (predates this field)';
  }

  return [
    `kind=${counterparty.kind}`,
    `name=${counterparty.name ?? 'null'}`,
    `contactHint=${counterparty.contactHint ?? 'null'}`
  ].join(', ');
}

export function renderOpportunitiesList(opportunities, filters) {
  const rows = opportunities
    .map(
      (o) => `<tr>
        <td><a href="/opportunities/${encodeURIComponent(o.id)}">${escapeHtml(o.title)}</a></td>
        <td>${escapeHtml(o.type)}</td>
        <td><span class="badge">${escapeHtml(o.status)}</span></td>
        <td>${escapeHtml(renderPursuitState(o))}</td>
        <td>${escapeHtml(renderCounterpartySummary(o.counterparty))}</td>
        <td>${o.score ? escapeHtml(o.score.value) : '-'}</td>
      </tr>`
    )
    .join('\n');

  return `
<form method="get" action="/opportunities">
  <label>Status <input type="text" name="status" value="${escapeHtml(filters.status ?? '')}"></label>
  <label>Type <input type="text" name="type" value="${escapeHtml(filters.type ?? '')}"></label>
  <label>Min score <input type="text" name="minScore" value="${escapeHtml(filters.minScore ?? '')}"></label>
  <button type="submit">Filter</button>
</form>
<table>
  <thead><tr><th>Title</th><th>Type</th><th>Status</th><th>Pursuit</th><th>Counterparty</th><th>Score</th></tr></thead>
  <tbody>${rows || '<tr><td colspan="6">No opportunities found.</td></tr>'}</tbody>
</table>`;
}

function renderChain(chain) {
  const items = chain
    .map(
      (event) =>
        `<li>${escapeHtml(event.occurredAt)} &mdash; [${escapeHtml(event.type)}] id=${escapeHtml(event.id)} causation=${escapeHtml(event.causationId ?? '-')} source=${escapeHtml(event.source)}<br>payload: ${escapeHtml(JSON.stringify(event.payload))}</li>`
    )
    .join('\n');
  return `<ol class="chain">${items || '<li>No events found.</li>'}</ol>`;
}

export function renderOpportunityDetail(opportunity, chain, { csrfToken, error, info } = {}) {
  const evidence = (opportunity.evidence ?? []).join(', ') || '-';
  const options = OPPORTUNITY_DECISIONS.map(
    (value) => `<option value="${value}">${value}</option>`
  ).join('');

  return `
${error ? `<p class="error">${escapeHtml(error)}</p>` : ''}
${info ? `<p>${escapeHtml(info)}</p>` : ''}
<dl>
  <dt>id</dt><dd>${escapeHtml(opportunity.id)}</dd>
  <dt>type</dt><dd>${escapeHtml(opportunity.type)}</dd>
  <dt>status</dt><dd>${escapeHtml(opportunity.status)}</dd>
  <dt>pursuable</dt><dd>${escapeHtml(opportunity.pursuable === true ? 'true' : 'false')}</dd>
  <dt>score</dt><dd>${opportunity.score ? escapeHtml(opportunity.score.value) : '-'}</dd>
  <dt>rationale</dt><dd>${opportunity.score ? escapeHtml(opportunity.score.rationale) : '-'}</dd>
  <dt>counterparty</dt><dd>${escapeHtml(renderCounterpartyDetail(opportunity.counterparty))}</dd>
  <dt>description</dt><dd>${escapeHtml(opportunity.description)}</dd>
  <dt>evidence</dt><dd>${escapeHtml(evidence)}</dd>
  <dt>correlationId</dt><dd>${escapeHtml(opportunity.correlationId)}</dd>
  <dt>createdAt</dt><dd>${escapeHtml(opportunity.createdAt)}</dd>
</dl>
<h2>Causal chain</h2>
${renderChain(chain)}
<h2>Decide</h2>
<form method="post" action="/opportunities/${encodeURIComponent(opportunity.id)}/decide">
  ${csrfField(csrfToken)}
  <fieldset>
    <label>Decision <select name="status">${options}</select></label>
    <label>Reason <input type="text" name="reason" required></label>
    <button type="submit">Decide</button>
  </fieldset>
</form>
<h2>e3d-pilot handoff</h2>
<form method="post" action="/opportunities/${encodeURIComponent(opportunity.id)}/propose-handoff">
  ${csrfField(csrfToken)}
  <fieldset>
    <label>Target repo path <input type="text" name="repo" required></label>
    <label>Reason <input type="text" name="reason" required></label>
    <button type="submit">Propose handoff</button>
  </fieldset>
</form>
<form method="post" action="/opportunities/${encodeURIComponent(opportunity.id)}/check-shipped">
  ${csrfField(csrfToken)}
  <button type="submit">Check shipped</button>
</form>
<h2>Record an outcome</h2>
<form method="post" action="/opportunities/${encodeURIComponent(opportunity.id)}/outcomes">
  ${csrfField(csrfToken)}
  <fieldset>
    <label>Type <select name="type">${OUTCOME_TYPES.map((t) => `<option value="${t}">${t}</option>`).join('')}</select></label>
    <label>Payload (JSON) <textarea name="payload" rows="3" cols="50">{}</textarea></label>
    <button type="submit">Record outcome</button>
  </fieldset>
</form>
<p><a href="/opportunities/${encodeURIComponent(opportunity.id)}/experience">View experience record</a></p>`;
}

function summarizeProposal(proposal) {
  const parts = [`type=${proposal.type}`, `level=${proposal.authorityLevel}`];
  if (typeof proposal.payload?.amount === 'number') {
    parts.push(`amount=${proposal.payload.amount}`);
  }
  if (proposal.type === 'send-outreach' && proposal.payload?.subject) {
    parts.push(`subject=${proposal.payload.subject}`);
  }
  const recipient = proposal.payload?.to ?? proposal.payload?.recipient;
  if (recipient) {
    parts.push(`recipient=${recipient}`);
  }
  return parts.join(', ');
}

function firstPresent(...values) {
  return values.find((value) => value !== undefined && value !== null && value !== '') ?? null;
}

function renderStressValue(value) {
  return escapeHtml(value ?? '-');
}

function renderFinancialStressSummary(payload) {
  const stage1 = payload?.pipeline?.stage1_research ?? {};
  const stage2 = payload?.pipeline?.stage2_crosscheck ?? {};
  const stage2b = payload?.pipeline?.stage2b_adjudication ?? payload?.pipeline?.stage2b ?? null;
  const stage3 = payload?.pipeline?.stage3_narrative ?? {};
  const draftScore = firstPresent(payload?.score_after, stage1?.score, stage1?.final_score);
  const stage2Score = firstPresent(stage2?.score, stage2?.final_score, stage2?.score_after);
  const agreement = firstPresent(stage2?.agreement, stage2?.verdict, stage2?.agrees_with_stage1);
  const flags = stage2?.flags ?? stage2?.issues ?? [];
  const flagsText = Array.isArray(flags) ? flags.join(', ') : flags;
  const headline = firstPresent(stage3?.headline, stage3?.title, stage3?.summary);
  const narrative = firstPresent(stage3?.narrative, stage3?.body, stage3?.summary);
  const stage1Evidence = firstPresent(stage1?.evidence, stage1?.summary, stage1?.rationale);
  const stage2Summary = firstPresent(stage2?.summary, stage2?.rationale, flagsText);
  const stage2bSummary = firstPresent(stage2b?.summary, stage2b?.rationale, stage2b?.decision);

  return `
<section>
  <h2>Financial stress triage</h2>
  <dl>
    <dt>run</dt><dd>${renderStressValue(payload?.run_id)}</dd>
    <dt>draft score</dt><dd>${renderStressValue(draftScore)}</dd>
    <dt>liquidity response</dt><dd>${renderStressValue(payload?.liquidity_response_after)}</dd>
    <dt>regime</dt><dd>${renderStressValue(payload?.regime_after)}</dd>
    <dt>crosscheck</dt><dd>${renderStressValue(agreement)}</dd>
    <dt>headline</dt><dd>${renderStressValue(headline)}</dd>
  </dl>
  <details>
    <summary>Three-stage breakdown</summary>
    <h3>Stage 1</h3>
    <dl>
      <dt>score</dt><dd>${renderStressValue(draftScore)}</dd>
      <dt>evidence</dt><dd>${escapeHtml(typeof stage1Evidence === 'string' ? stage1Evidence : JSON.stringify(stage1Evidence ?? '-'))}</dd>
    </dl>
    <h3>Stage 2</h3>
    <dl>
      <dt>score</dt><dd>${renderStressValue(stage2Score)}</dd>
      <dt>flags</dt><dd>${escapeHtml(typeof stage2Summary === 'string' ? stage2Summary : JSON.stringify(stage2Summary ?? '-'))}</dd>
    </dl>
    ${
      stage2b
        ? `<h3>Stage 2b</h3><p>${escapeHtml(typeof stage2bSummary === 'string' ? stage2bSummary : JSON.stringify(stage2bSummary ?? '-'))}</p>`
        : ''
    }
    <h3>Stage 3</h3>
    <p>${escapeHtml(narrative ?? '-')}</p>
  </details>
</section>`;
}

function renderStressCorrectionFields(proposal) {
  if (proposal.type !== 'publish-stress-change') {
    return '';
  }
  return `
  <label>Final score <input type="number" name="finalScore" step="any" value="${escapeHtml(proposal.payload?.score_after ?? '')}"></label>
  <label>Final liquidity response <input type="text" name="finalLiquidityResponse" value="${escapeHtml(proposal.payload?.liquidity_response_after ?? '')}"></label>
  <label>Final regime <input type="text" name="finalRegime" value="${escapeHtml(proposal.payload?.regime_after ?? '')}"></label>
  <label>Correction note <input type="text" name="correctionNote"></label>`;
}

function renderProposalRows(proposals) {
  return proposals
    .map(
      (p) => `<tr>
        <td><a href="/proposals/${encodeURIComponent(p.id)}">${escapeHtml(p.id)}</a></td>
        <td>${escapeHtml(p.type)}</td>
        <td>${escapeHtml(p.authorityLevel)}</td>
        <td><span class="badge">${escapeHtml(p.status)}</span></td>
        <td>${escapeHtml(summarizeProposal(p))}</td>
      </tr>`
    )
    .join('\n');
}

export function renderProposalsList(groups, filters) {
  const sections = groups
    .map((group) => {
      const heading =
        group.proposals.length > 1
          ? `<p><strong>${escapeHtml(group.label)}</strong> (${group.proposals.length} drafts, correlation ${escapeHtml(group.correlationId)})</p>`
          : '';
      return `${heading}
<table>
  <thead><tr><th>Id</th><th>Type</th><th>Level</th><th>Status</th><th>Summary</th></tr></thead>
  <tbody>${renderProposalRows(group.proposals)}</tbody>
</table>`;
    })
    .join('\n');

  return `
<form method="get" action="/proposals">
  <label>Status <input type="text" name="status" value="${escapeHtml(filters.status ?? '')}"></label>
  <label>Type <input type="text" name="type" value="${escapeHtml(filters.type ?? '')}"></label>
  <button type="submit">Filter</button>
</form>
${sections || '<table><tbody><tr><td>No proposals found.</td></tr></tbody></table>'}`;
}

export function renderActionsList(actions) {
  const rows = actions
    .map(
      (action) => `<tr>
        <td>${escapeHtml(action.occurredAt)}</td>
        <td>${escapeHtml(action.type)}</td>
        <td>${escapeHtml(action.summary)}</td>
        <td>${action.proposalId ? `<a href="/proposals/${encodeURIComponent(action.proposalId)}">${escapeHtml(action.proposalId)}</a>` : '-'}</td>
        <td>${action.opportunityId ? `<a href="/opportunities/${encodeURIComponent(action.opportunityId)}">${escapeHtml(action.opportunityId)}</a>` : '-'}</td>
      </tr>`
    )
    .join('\n');

  return `
<p>Observational log of actions that have actually fired. Approving already happened upstream at the Proposal - nothing here is itself something to approve.</p>
<table>
  <thead><tr><th>When</th><th>Type</th><th>Summary</th><th>Proposal</th><th>Opportunity</th></tr></thead>
  <tbody>${rows || '<tr><td colspan="5">No actions have fired yet.</td></tr>'}</tbody>
</table>`;
}

export function renderProposalDetail(proposal, chain, siblings, { csrfToken, error, executed } = {}) {
  const needsConfirm = proposal.status === 'approved' && proposal.authorityLevel >= AUTHORITY_LEVELS.FINANCIAL_ACTION;
  const decideForms =
    proposal.status === 'pending'
      ? `
<form class="inline" method="post" action="/proposals/${encodeURIComponent(proposal.id)}/approve">
  ${csrfField(csrfToken)}
  <label>Reason <input type="text" name="reason" required></label>
  ${renderStressCorrectionFields(proposal)}
  <button type="submit">Approve${proposal.authorityLevel >= AUTHORITY_LEVELS.FINANCIAL_ACTION ? ' (requires confirm)' : ''}</button>
</form>
<form class="inline" method="post" action="/proposals/${encodeURIComponent(proposal.id)}/reject">
  ${csrfField(csrfToken)}
  <label>Reason <input type="text" name="reason" required></label>
  <button type="submit">Reject</button>
</form>`
      : '';
  const confirmForm = needsConfirm
    ? `
<form method="post" action="/proposals/${encodeURIComponent(proposal.id)}/confirm">
  ${csrfField(csrfToken)}
  <p>This proposal is authority level ${proposal.authorityLevel} and has been approved but not yet executed. Confirm to fire the action.</p>
  <button type="submit">Confirm and execute</button>
</form>`
    : '';

  const siblingSection =
    siblings.length > 1
      ? `
<h2>Sibling drafts</h2>
<p>These proposals are alternatives for the same outreach opportunity.</p>
<table>
  <thead><tr><th>Id</th><th>Status</th><th>Provider</th><th>Summary</th></tr></thead>
  <tbody>${siblings
    .map(
      (sibling) => `<tr>
        <td><a href="/proposals/${encodeURIComponent(sibling.id)}">${escapeHtml(sibling.id)}</a></td>
        <td>${escapeHtml(sibling.status)}</td>
        <td>${escapeHtml(sibling.proposedBy?.provider ?? '-')}</td>
        <td>${escapeHtml(summarizeProposal(sibling))}</td>
      </tr>`
    )
    .join('\n')}</tbody>
</table>`
      : '';

  return `
${error ? `<p class="error">${escapeHtml(error)}</p>` : ''}
${executed ? '<p><strong>Action executed.</strong></p>' : ''}
<dl>
  <dt>id</dt><dd>${escapeHtml(proposal.id)}</dd>
  <dt>type</dt><dd>${escapeHtml(proposal.type)}</dd>
  <dt>authorityLevel</dt><dd>${escapeHtml(proposal.authorityLevel)}</dd>
  <dt>status</dt><dd>${escapeHtml(proposal.status)}</dd>
  <dt>proposedBy</dt><dd>${escapeHtml(JSON.stringify(proposal.proposedBy))}</dd>
  ${proposal.type === 'publish-stress-change' ? '' : `<dt>payload</dt><dd>${escapeHtml(JSON.stringify(proposal.payload))}</dd>`}
  <dt>correlationId</dt><dd>${escapeHtml(proposal.correlationId)}</dd>
  <dt>createdAt</dt><dd>${escapeHtml(proposal.createdAt)}</dd>
</dl>
${proposal.type === 'publish-stress-change' ? renderFinancialStressSummary(proposal.payload) : ''}
${siblingSection}
<h2>Causal chain</h2>
${renderChain(chain)}
<h2>Decide</h2>
${decideForms || `<p>Status: ${escapeHtml(proposal.status)}</p>`}
${confirmForm}`;
}

export function renderOutcomesList(outcomes) {
  const rows = outcomes
    .map(
      (outcome) => `<tr>
        <td>${escapeHtml(outcome.occurredAt)}</td>
        <td>${escapeHtml(outcome.type)}</td>
        <td>${escapeHtml(outcome.correlationId)}</td>
        <td>${escapeHtml(JSON.stringify(outcome.payload))}</td>
      </tr>`
    )
    .join('\n');

  return `
<p>Real-world facts recorded about what actually happened - distinct from and never a substitute for approval Decisions.</p>
<table>
  <thead><tr><th>When</th><th>Type</th><th>correlationId</th><th>Payload</th></tr></thead>
  <tbody>${rows || '<tr><td colspan="4">No outcomes recorded yet.</td></tr>'}</tbody>
</table>`;
}

export function renderExperience(experience) {
  return `
<dl>
  <dt>correlationId</dt><dd>${escapeHtml(experience.correlationId)}</dd>
  <dt>context</dt><dd>${escapeHtml(experience.context)}</dd>
  <dt>role</dt><dd>${escapeHtml(experience.role ?? '-')}</dd>
  <dt>model</dt><dd>${escapeHtml(experience.model ?? '-')}</dd>
  <dt>originatingEvent</dt><dd>[${escapeHtml(experience.originatingEvent.type)}] ${escapeHtml(experience.originatingEvent.id)}</dd>
  <dt>evidence</dt><dd>${experience.evidence.length} item(s)</dd>
  <dt>proposal</dt><dd>${experience.proposal ? escapeHtml(experience.proposal.type) : '-'}</dd>
  <dt>decision</dt><dd>${experience.decision ? escapeHtml(experience.decision.decision ?? experience.decision.status ?? '') : '-'}</dd>
  <dt>action</dt><dd>${experience.action ? escapeHtml(experience.action.type) : '-'}</dd>
  <dt>outcome</dt><dd>${experience.outcome ? escapeHtml(experience.outcome.type) : '-'}</dd>
  <dt>latencyMs</dt><dd>${experience.latencyMs ?? '-'}</dd>
  <dt>costEstimate</dt><dd>${experience.costEstimate ?? '-'}</dd>
</dl>`;
}

function formatRate(rate) {
  return rate === null ? 'n/a (no data)' : `${(rate * 100).toFixed(1)}%`;
}

export function renderMetrics(metrics) {
  const typeRows = Object.entries(metrics.opportunitiesDiscovered.byType)
    .map(([type, count]) => `<li>${escapeHtml(type)}: ${count}</li>`)
    .join('');
  const budgetRows = (metrics.budget ?? [])
    .map((status) =>
      status.unlimited
        ? `<li>${escapeHtml(status.provider)}: unlimited</li>`
        : `<li>${escapeHtml(status.provider)}: spent=${status.settled} allocated=${status.limit} outstanding=${status.outstanding} remaining=${status.remaining}</li>`
    )
    .join('');
  const latencyRows = Object.entries(metrics.latencyByRoleModel)
    .map(([key, ms]) => `<li>${escapeHtml(key)}: ${Math.round(ms)}ms avg</li>`)
    .join('');
  const costRows = Object.entries(metrics.costByRoleModel)
    .map(([key, cost]) => `<li>${escapeHtml(key)}: ${cost}</li>`)
    .join('');

  return `
${metrics.since ? `<p>Since: ${escapeHtml(metrics.since)}</p>` : ''}
<h2>Opportunities discovered</h2>
<p>Total: ${metrics.opportunitiesDiscovered.total}</p>
<ul>${typeRows || '<li>(none)</li>'}</ul>
<h2>Opportunity acceptance</h2>
<p>pursuing=${metrics.opportunityAcceptance.pursuing} no-value=${metrics.opportunityAcceptance.noValue} rate=${formatRate(metrics.opportunityAcceptance.rate)}</p>
<h2>Proposal rejection (human override rate)</h2>
<p>approved=${metrics.proposalRejection.approved} rejected=${metrics.proposalRejection.rejected} rate=${formatRate(metrics.proposalRejection.rate)}</p>
<h2>Outreach funnel</h2>
<p>sent=${metrics.outreach.sent} replied=${metrics.outreach.replied} (${formatRate(metrics.outreach.responseRate)})<br>
meetingsBooked=${metrics.outreach.meetingsBooked} (${formatRate(metrics.outreach.meetingBookedRate)})<br>
dealsWon=${metrics.outreach.dealsWon} (${formatRate(metrics.outreach.dealWinRate)})</p>
<h2>Revenue and cost</h2>
<p>Revenue attributable: ${metrics.revenueAttributable}</p>
<p>Useful opportunities: ${metrics.usefulOpportunityCount}</p>
<p>Cost per useful opportunity: ${metrics.costPerUsefulOpportunity === null ? 'n/a (no cost data recorded yet)' : metrics.costPerUsefulOpportunity}</p>
<h3>Budget status</h3>
<ul>${budgetRows || '<li>(no provider budgets configured)</li>'}</ul>
<h3>Latency by role/model</h3>
<ul>${latencyRows || '<li>(no data)</li>'}</ul>
<h3>Cost by role/model</h3>
<ul>${costRows || '<li>(no cost data recorded yet)</li>'}</ul>
<form method="get" action="/metrics">
  <label>Since <input type="text" name="since" value="${escapeHtml(metrics.since ?? '')}" placeholder="ISO date"></label>
  <button type="submit">Filter</button>
</form>`;
}
