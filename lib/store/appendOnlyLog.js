...
export function withFileLock(lockPath, fn) {
  ...
  const release = () => releaseFileLock(lockPath, fd);
...
export async function withAsyncFileLock(lockPath, fn) {
  const fd = await acquireFileLockAsync(lockPath);
  try {
    return await fn();
  } finally {
...
async function acquireFileLockAsync(lockPath) {
  const deadline = Date.now() + LOCK_TIMEOUT_MS;
