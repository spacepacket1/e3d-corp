...
const COMMANDS = [
  ...
  ['event verify [--head <hash> --count <n>]', 'Verify the hash chain, published anchors, and optionally an emailed anchor'],
  ...
  ['opportunities propose-handoff <id> --repo <path> --reason <reason>', 'Propose an e3d-pilot handoff for a "pursuing" opportunity'],
  ['opportunities check-shipped <id>', 'Re-query futco-mcp for evidence a related capability now exists'],
  ...
  ['proposals confirm <id>', 'Confirm and fire an approved level-3/4 proposal (the required second step)'],
  ['web [--instance <name>] [--port <n>]', 'Start the web UI (basic auth required; refuses to start without it)'],
  ['outcomes record --correlation <id> --type <type> --payload <json> [--occurred-at <iso>]', 'Record a real-world Outcome for a chain'],
  ['experience show <correlationId>', 'Render one fully assembled Experience record for a chain'],
  ['evaluate report [--since <date>]', 'Render evaluation metrics computed over real events/experience data']
];
...
export function formatHelp() {
  ...
  lines.push(
    '',
    'Implemented so far: `config validate`, `event add`, `event log`, `event verify`, `anchor publish`, `run`, `pursue`, `opportunities list`, ' +
      ...
      '`experience show`, `evaluate report`. The remaining commands are documented stubs for later phases.'
  );
  return lines.join('\n');
}
...
function evaluateReportCommand(args) {
  ...
  const metrics = computeMetrics(dataDir, { since: typeof flags.since === 'string' ? flags.since : undefined });
