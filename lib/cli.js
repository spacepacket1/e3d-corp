...
export function formatHelp() {
  ...
  lines.push(
    '',
    'Implemented so far: `config validate`, `event add`, `event log`, `run`, `opportunities list`, ' +
      '`opportunities show`, `opportunities decide`, `proposals list`, `proposals show`, `proposals approve`, ' +
      '`proposals reject`. The remaining commands are documented stubs for later phases.'
  );
  return lines.join('\n');
}
...
async function proposalsDecideCommand(args, decisionValue) {
  ...
  const result = await decideProposal(dataDir, id, decisionValue, reason, resolveDecidedBy(flags), 'cli');
