...
// Deterministic keyword-overlap matching against the knowledge base's real repo
...
export async function checkCapabilityShipped({ dataDir, opportunityId, listRepos } = {}) {
  ...
  if (result.status !== 'ok') {
    throw new Error(`knowledge-base MCP list_repos unavailable: ${result.reason ?? result.summary ?? 'unknown error'}`);
  }
