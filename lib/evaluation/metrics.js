...
function computeLatencyAndCostByRoleModel(experiences) {
  const latencyBuckets = {};
  const costBuckets = {};
  for (const experience of experiences) {
    const key = `${experience.role ?? 'unknown'}:${experience.model ?? 'unknown'}`;
    if (typeof experience.latencyMs === 'number') {
      (latencyBuckets[key] ??= []).push(experience.latencyMs);
    }
    if (typeof experience.costEstimate === 'number') {
      (costBuckets[key] ??= []).push(experience.costEstimate);
    }
  }
  return {
...
export function computeMetrics(dataDir, { since } = {}) {
  const experiences = listLatestExperience(dataDir).filter((experience) => afterSince(experience.originatingEvent?.occurredAt, since));
  const { usefulOpportunityCount, costPerUsefulOpportunity } = computeCostPerUsefulOpportunity(experiences);
  const { latencyByRoleModel, costByRoleModel } = computeLatencyAndCostByRoleModel(experiences);
