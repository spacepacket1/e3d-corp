// The shared local model server serializes all generation globally (one
// request at a time across every pipeline that hits it), and prefill cost on
// large evidence-heavy prompts routinely runs 90-300s once queued behind
// another caller's request. 60s was too tight for that reality, not a bug in
// the request itself.
const DEFAULT_LOCAL_TIMEOUT_MS = 300_000;
const DEFAULT_LOCAL_MAX_TOKENS = 2_000;
...
export function resolveLocalProviderSettings(providerConfig) {
  ...
  return {
    ...
    maxTokens: providerConfig.maxTokens ?? DEFAULT_LOCAL_MAX_TOKENS
  };
}
...
export function createLocalLlmClient(instanceConfig) {
  ...
  const { baseUrl, model, timeoutMs, maxTokens } = resolveLocalProviderSettings(providerConfig);
  return ({ systemPrompt, userPrompt }) =>
    callLocalLlm({ baseUrl, model, systemPrompt, userPrompt, timeoutMs, maxTokens });
}
