...
// below.
export async function decideProposal(dataDir, id, decision, reason, decidedBy, via) {
  ...
  if (decision === 'approved' && proposal.authorityLevel === AUTHORITY_LEVELS.EXTERNAL_ACTION) {
    const executor = getActionExecutor(proposal.type);
    if (executor) {
      const approvedProposal = { ...proposal, status: 'approved' };
...
export async function confirmAndExecute(dataDir, proposalId, confirmedBy, via) {
